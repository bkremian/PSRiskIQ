...
#>
    ...
    process {
        ...
        foreach ($Endpoint in (($ParamDetail.Attributes.ParameterSetName | Group-Object).Name |
            ...
            if ($RiskIQ.Endpoint($Endpoint).Description) {
                # Add endpoint description
                $HeaderText += "  $($RiskIQ.Endpoint($Endpoint).Description)"
            }
            if ($RiskIQ.Endpoint($Endpoint).Permission) {
                # Add permission
                $HeaderText += ("`n    Requires" +
                " $((($RiskIQ.Endpoint($Endpoint)).Permission))")
            }
            # Output header text
            $HeaderText
